...
function ContactList(props) {
  ...
  const contactItems =
    filteredContacts.length < 1 ? contacts : filteredContacts;
    ...
    if (searchQuery) {
      filtered = contacts.filter((contact) => {
        ...
        Object.values(contact).forEach((val) => {
          if (expr.test(val)) found = true;
        });
        if (found) return contact;
      });
    }
    if (filtered && filtered.length > 0) setFilteredContacts(filtered);
    else if (filtered && filtered.length <= 0) setHasError(true);
  });
  useEffect(() => {
    let isMounted = true;
  ...
  useEffect(() => {
    ...
    if (!query) setHasError(false);
  }, [query]);

  return (
    <Container fluid={true}>
      <Row className="mb-4 mt-4">
        <h1 className="text-nowrap ml-auto mr-auto header-text">
          Address Book
        </h1>
      </Row>
      <Row className="mb-4 mt-4">
      ...
      >
        <ModalHeader>
          <Row className="w-100">
            <Col>
              {" "}
              <h4 className="text-nowrap mr-4">Contact Details</h4>
            </Col>
